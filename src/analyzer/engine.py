...
PRICING_SYSTEM_PROMPT = """You are a seasoned VP of Competitive Intelligence and Pricing Strategy.
Your job is to analyze raw diffs from competitor pricing pages and extract high-value strategic signals.

Evaluate:
1. What specifically changed in the pricing tiers, price points, billing intervals, or feature gating?
...
4. Assess strategic impact: HIGH (significant price hike, new tier, or gating core feature), MEDIUM (notable limit or packaging change), or LOW (minor adjustments).
5. Predict strategic intent (why they did this).
6. Give actionable counter-recommendations for our sales/marketing/product teams.
...
Respond strictly in valid JSON format matching this schema:
{
  ...
  "counter_strategy": "recommended action for our team"
}
"""

PRODUCT_SYSTEM_PROMPT = """You are a seasoned Chief Product Officer and Competitive Strategist.
Your job is to analyze competitor changelogs and release notes to assess market threat and parity.

Evaluate:
1. What new capabilities, integrations, or workflows did they launch?
...
3. Threat level: HIGH (major new capability that challenges our differentiator), MEDIUM (solid feature or parity catch-up), LOW (routine update or polish).
4. Competitive implication: Does this close a gap, leapfrog an existing feature, or target a new persona?
5. Counter-strategy: Recommended roadmap or positioning adjustments.
...
Respond strictly in valid JSON format matching this schema:
{
  ...
  "competitive_implication": "market position impact",
  "counter_strategy": "recommended roadmap or positioning action"
}
"""


class IntelligenceEngine:
    def __init__(self, db: Database, client: GeminiClient):
        self.db = db
        self.client = client
    ...
    def analyze_pricing_event(self, event: Dict[str, Any]) -> PricingAnalysisResult:
        ...
        json_data = self.client.generate_json(prompt=prompt, system_instruction=PRICING_SYSTEM_PROMPT)
    ...
    def analyze_product_event(self, event: Dict[str, Any]) -> ProductUpdateAnalysisResult:
        ...
        json_data = self.client.generate_json(prompt=prompt, system_instruction=PRODUCT_SYSTEM_PROMPT)
